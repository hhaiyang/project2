...
//	CGSize labelSize = [label.text sizeWithFont:label.font];
    ...
    CGRect detailsLabelSize = [detailsLabel.text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil];
    
    totalSize.width = MAX(totalSize.width, detailsLabelSize.size.width);
    totalSize.height += detailsLabelSize.size.height;
    
    if (detailsLabelSize.size.height > 0.f && (indicatorF.size.height > 0.f || labelSize.height > 0.f)) {
        totalSize.height += kPadding;
    }
...
//	detailsLabelF.size = detailsLabelSize;
    detailsLabelF.origin.x = roundf((bounds.size.width - detailsLabelSize.size.width) / 2) + xPos;
    detailsLabelF.size = detailsLabelSize.size;
