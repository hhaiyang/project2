...
@implementation guangxierPopView
{
    NSArray *buttonItems;
    CGFloat popViewX;
    CGFloat popViewY;
...
{
    _PopBorderColor7 = PopBorderColor;
...
{
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(chooseButton7:)]) {
        [self.delegate chooseButton7:sender.titleLabel.text];
        [self showPopView7];
    }
}
/**
...
- (void)loadUI:(UIView *)superView
{
    ...
    self.frame = CGRectMake(popViewX, popViewY-190, superView.frame.size.width, 0);
    [self setButton];
